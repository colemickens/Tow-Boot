...
let
  ...
  configTxt =
    let
      ...
      opt = (f: p:
        let chk =
          if (builtins.hasAttr p cfg_rpi && cfg_rpi."${p}" != null)
          then (f cfg_rpi."${p}")
          else null;
        in
      ...
      configTxtPi4 = (
        ...
        ''
      ) + (lib.optionalString (cfg_rpi.enable_vc4_kms && !cfg_rpi.upstream_kernel) ''
        # gpu_mem=256                # Tow-Boot.config.rpi.enable_vc4_kms
      '') + (lib.optionalString (cfg_rpi.enable_vc4_kms && cfg_rpi.upstream_kernel) ''
        # gpu_mem=256                # Tow-Boot.config.rpi.enable_vc4_kms
      '')
      + (lib.optionalString ((cfg_rpi.hdmi_enable_4kp60 != null) && cfg_rpi.hdmi_enable_4kp60) ''
        hdmi_enable_4kp60=1
    ...
    pkgs.writeText "config.txt"
      (''
        ...
        # eeprom.package: ${cfg_eeprom.package.name}
        ...
        enable_uart=1
        avoid_warnings=1
        upstream_kernel=${toBooint cfg_rpi.upstream_kernel}
        kernel=${final_binary}
      ''
      ...
      + (lib.optionalString (cfg_rpi.enable_watchdog != null && cfg_rpi.enable_watchdog) ''
        dtparam=watchdog=on
      '')
      + (lib.optionalString (cfg_rpi.upstream_kernel != null && !cfg_rpi.upstream_kernel) ''
  ...
  eepromFiles = pkgs.runCommandNoCC "rpi4-eeprom" { } ''
    ...
    dir="${cfg_eeprom.package}/share/rpi-eeprom/stable"
    orig="$(ls $dir | grep pieeprom | sort | tail -1)"

    ${cfg_eeprom.package}/bin/rpi-eeprom-config \
      --config "${bootconfTxt}" \
      --out $out/pieeprom.upd \
        "$(readlink -f $dir/$orig)"
              
    ${cfg_eeprom.package}/bin/rpi-eeprom-digest \
      -i $out/pieeprom.upd \
      -o $out/pieeprom.sig
  '';
        ...
        outputs.extra.scripts =
          let
            ...
            updateEeprom = pkgs.writeShellScriptBin "tow-boot-rpi-update-eeprom" ''
              #
              # UPDATE EEPROM
              sudo ${cfg_eeprom.package}/bin/rpi-eeprom-update -r || true
              sudo env BOOTFS=/boot/firmware \
                ${cfg_eeprom.package}/bin/rpi-eeprom-update \
